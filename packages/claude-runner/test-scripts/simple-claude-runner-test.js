...
import { readOnlyTools } from '../dist/index.js'
...
async function main() {
  ...
  if (!process.env.LINEAR_API_TOKEN) {
    ...
    console.error('Make sure .env file exists in the claude-runner package directory')
    process.exit(1)
  }
  ...
  const config = {
    ...
    allowedTools: [
      ...
      "mcp__linear__getViewer",
      "mcp__linear__getOrganization",
      "mcp__linear__getUsers",
      "mcp__linear__getLabels",
      "mcp__linear__getTeams",
      "mcp__linear__getProjects",
      "mcp__linear__createProject",
      "mcp__linear__getIssues",
      "mcp__linear__getIssueById",
      "mcp__linear__searchIssues",
      "mcp__linear__createIssue",
      "mcp__linear__updateIssue",
      "mcp__linear__createComment",
      "mcp__linear__addIssueLabel",
      "mcp__linear__removeIssueLabel",
      "mcp__linear__assignIssue",
      "mcp__linear__subscribeToIssue",
      "mcp__linear__convertIssueToSubtask",
      "mcp__linear__createIssueRelation",
      "mcp__linear__archiveIssue",
      "mcp__linear__setIssuePriority",
      "mcp__linear__transferIssue",
      "mcp__linear__duplicateIssue",
      "mcp__linear__getIssueHistory",
      "mcp__linear__getComments",
      "mcp__linear__updateProject",
      "mcp__linear__addIssueToProject",
      "mcp__linear__getProjectIssues",
      "mcp__linear__getCycles",
      "mcp__linear__getActiveCycle",
      "mcp__linear__addIssueToCycle"
    ...
    systemPrompt: 'You are a helpful assistant. Please be concise and clear in your responses.',
          ...
          if (message.message?.content) {
            ...
            message.message.content.forEach((block, index) => {
              if (block.type === 'tool_result') {
                console.log(`   ${index + 1}. Tool result for: ${block.tool_use_id}`)
...
If no, tell me what MCP-related tools you see (if any).
  ...
  try {
    ...
    console.log(`🔄 Final status: ${sessionInfo.isRunning ? 'Running' : 'Completed'}`)
