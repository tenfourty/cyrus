...
async function main() {
  ...
  if (!existsSync(workingDirectory)) {
    ...
    console.log(`[EdgeWorker-like] Created working directory: ${workingDirectory}`)
  } else {
    console.log(`[EdgeWorker-like] Using existing directory: ${workingDirectory}`)
  }
  ...
  const config = {
    ...
    systemPrompt: 'You are an expert software engineer. Focus on practical solutions.',
  ...
  console.log('[EdgeWorker-like] Starting new streaming session for issue CEE-739')
...
Let me know what you find and we can work together on implementing a solution.`
    ...
    for (let i = 0; i < 120; i++) {
      if (completed) {
        console.log(`🎉 Session completed after ${i + 1} seconds with ${messageCount} messages`)
        break
      }
      
      // Show progress every 10 seconds
      if ((i + 1) % 10 === 0) {
        console.log(`⏳ Still waiting... ${i + 1}s (streaming: ${runner.isStreaming()}, running: ${runner.isRunning()})`)
      }
    ...
    if (!completed) {
      console.log('🚨 SESSION IS HANGING - This reproduces the production issue!')
      console.log(`🔍 Final state: streaming=${runner.isStreaming()}, running=${runner.isRunning()}`)
