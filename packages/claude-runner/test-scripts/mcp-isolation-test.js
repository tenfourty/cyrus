...
async function testMCPConfig(testName, mcpConfig, mcpConfigPath = null) {
  ...
  const config = {
    ...
    onMessage: (message) => {
      if (message.type === 'system' && message.mcp_servers) {
        console.log('✅ MCP servers loaded:', message.mcp_servers.map(s => s.name))
      }
    },
  ...
  try {
    ...
    if (error.message === 'TIMEOUT') {
      console.log(`❌ ${testName} HUNG after ${elapsed}ms - THIS IS THE CULPRIT!`)
    } else {
...
async function main() {
  ...
  await testMCPConfig('Only file-based MCP servers', null, ['/Users/agentops/code/ceedarmcpconfig.json'])
  
  // Test 4: All MCP servers (should hang)
  await testMCPConfig('All MCP servers (production config)', {
    "linear": {
      "type": "stdio", 
      "command": "npx",
      "args": ["-y", "@tacticlaunch/mcp-linear"],
