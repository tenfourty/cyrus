...
async function testSubagentFunctionality() {
  ...
  console.log('💡 Using Claude Code SDK authentication (run `claude auth` if not set up)')

  // Create test workspace
  const testWorkspaceDir = resolve(tmpdir(), `claude-runner-subagent-test-${Date.now()}`)
...
`
    ...
    writeFileSync(resolve(testWorkspaceDir, 'sample-data.txt'), 'This file contains test data for subagent functionality verification.')
        ...
        if (message.type === 'assistant' && message.message?.content) {
          for (const block of message.message.content) {
            if (block.type === 'tool_use' && block.name === 'Task') {
              ...
              console.log(`📋 Task description: ${block.input?.description || 'N/A'}`)
              console.log(`🎯 Subagent type: ${block.input?.subagent_type || 'N/A'}`)
            }
          }
        }
...
`
    ...
    if (success) {
      ...
      if (!foundSubagentResponse) console.log('   - Subagent response not detected')
    }
    ...
    if (!success) {
      ...
      const logFile = resolve(__dirname, `subagent-test-log-${Date.now()}.json`)
