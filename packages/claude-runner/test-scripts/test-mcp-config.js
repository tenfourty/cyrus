...
async function main() {
  try {
    ...
    try {
      ...
      const ceedarRepo = edgeConfig.repositories?.find(repo => repo.name === 'ceedar')
      allowedTools = ceedarRepo?.allowedTools || []
      console.log('Found allowed tools for ceedar:', allowedTools)
    } catch (err) {
      console.log('Could not read edge config, using default tools:', err.message)
    ...
    const prompt = "Use the mcp__ceedardb__query tool to look up business id 113 in the ceedardb database"
