...
async function testWorkingDir(workingDirectory, testName) {
  ...
  try {
    ...
    const sessionInfo = await Promise.race([
      runner.startStreaming('Hello, please tell me what directory you are in and list the files.'),
...
async function main() {
  ...
  await testWorkingDir('/Users/agentops/code/ceedar-new-workspaces/CEE-739', 'Production directory (CEE-739)')
  
  // Test 3: Different CEE directory  
  await testWorkingDir('/Users/agentops/code/ceedar-new-workspaces/CEE-test', 'Different CEE directory')
  
  // Test 4: Parent ceedar-new-workspaces directory
  await testWorkingDir('/Users/agentops/code/ceedar-new-workspaces', 'Parent workspaces directory')
