...
describe("with allowedDirectories (attachments dir, repo paths, etc.)", () => {
	...
	it("carves out the per-repo Claude auto-memory directory so MEMORY.md updates work", () => {
		...
		mockHome(HOME, {
			...
			".claude": dir({
				agents: dir(),
				projects: dir({
					"-home-alice--cyrus-repos-repoA": dir({
						memory: dir({
							"MEMORY.md": file(),
							"feedback_x.md": file(),
						}),
			...
			".cyrus": dir({
				worktrees: dir({ "ENG-1": dir({ repoA: dir() }) }),
				repos: dir({ repoA: dir() }),
			}),
		});

		const cwd = `${HOME}/.cyrus/worktrees/ENG-1/repoA`;
		const memoryDir = `${HOME}/.claude/projects/-home-alice--cyrus-repos-repoA/memory`;
		...
		check(denied, HOME)
			...
			.denies(".claude/projects/-home-alice--cyrus-repos-repoA/abc-1234.jsonl") // session transcript stays private
			...
			.allows(".claude/projects/-home-alice--cyrus-repos-repoA")
			.allows(".claude/projects/-home-alice--cyrus-repos-repoA/memory")
			.allows(
				".claude/projects/-home-alice--cyrus-repos-repoA/memory/MEMORY.md",
			);
	});
	...
	it("supports carve-outs for multiple repos in a multi-repo session", () => {
		mockHome(HOME, {
			".claude": dir({
				projects: dir({
					...
					"-home-alice--cyrus-repos-repoB": dir({
						memory: dir({ "MEMORY.md": file() }),
						"def.jsonl": file(),
					}),
					"-home-alice--cyrus-repos-other": dir({
			...
			".cyrus": dir({
				worktrees: dir({ "ENG-1": dir({ repoA: dir(), "repoB": dir() }) }),
			}),
		});

		const cwd = `${HOME}/.cyrus/worktrees/ENG-1/repoA`;
		const coveMemory = `${HOME}/.claude/projects/-home-alice--cyrus-repos-repoA/memory`;
		const ovhMemory = `${HOME}/.claude/projects/-home-alice--cyrus-repos-repoB/memory`;
		...
		check(denied, HOME)
			.denies(".claude/projects/-home-alice--cyrus-repos-repoA/abc.jsonl")
			.denies(".claude/projects/-home-alice--cyrus-repos-repoB/def.jsonl")
			.denies(".claude/projects/-home-alice--cyrus-repos-other") // a different project's memory stays private
			.allows(".claude/projects/-home-alice--cyrus-repos-repoA/memory/MEMORY.md")
			.allows(
				".claude/projects/-home-alice--cyrus-repos-repoB/memory/MEMORY.md",
			);
	});
});
