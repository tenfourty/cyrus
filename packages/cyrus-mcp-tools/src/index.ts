...
import { getLinearApiToken, logError, logInfo } from "./utils/config.js";
	...
	try {
		...
		const server = await runMCPServer({
			...
			handleRequest: async (req: { name: string; args: unknown }) => {
				...
				// Check if handlers is empty (no tools available yet)
				if (Object.keys(handlers).length === 0) {
					throw new Error(
						`No tools available yet. Upload file tool will be added soon.`,
					);
				}
