...
describe("resolveGitLabBotIdentity", () => {
	...
	it("returns id and username from GET /api/v4/user", async () => {
		globalThis.fetch = vi.fn().mockResolvedValue({
			ok: true,
			status: 200,
			json: async () => ({ id: 42, username: "repo-a-agent-bot" }),
		...
		expect(identity).toEqual({ id: 42, username: "repo-a-agent-bot" });
	...
	it("throws with status when GitLab returns non-2xx", async () => {
		...
		await expect(resolveGitLabBotIdentity({ token: "bad" })).rejects.toThrow(
			/401/,
		);
	});
