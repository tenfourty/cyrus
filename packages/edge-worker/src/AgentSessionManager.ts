...
export type AgentSessionManagerEvents = {
	 ...
	 * EdgeWorker listens to this (NOT session_terminal) to post the
	 * session rollup, because telemetry totals are not available until
	 * after the result entry's metadata is filled in. session_terminal
	 * fires earlier (at status-transition time) for drain tracking.
	 */
