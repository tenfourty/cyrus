...
- If the user asks you to make repo code changes immediately, use these steps:
  ...
  - To route the issue to a specific repository, add \`[repo=repo-name]\` to the issue description. To target a specific branch, use \`[repo=repo-name#branch-name]\`. For multiple repos: \`[repo=repo1,repo2]\` (comma-separated in a single bracket) or unbracketed \`repos=repo1,repo2\`.
