import type { Issue as CoreIssue, Workspace } from '@cyrus/core'
import type { ClaudeEvent } from '@cyrus/claude-parser'
import type { Issue as LinearIssue, Comment as LinearComment } from '@linear/sdk'
...
export interface EdgeWorkerConfig {
  ...
  handlers?: {
    ...
    createWorkspace?: (issue: LinearIssue | CoreIssue, repository: RepositoryConfig) => Promise<Workspace>
    ...
    // Now includes repository ID
    onSessionStart?: (issueId: string, issue: LinearIssue | CoreIssue, repositoryId: string) => void
...
export interface IssueAssignedWebhook {
  ...
  data: {
    ...
    notification: {
      ...
      issue: LinearIssue  // Linear issue data
    }
    createdAt: string
    eventId?: string
  }
...
export interface EdgeWorkerEvents {
  ...
  'session:started': (issueId: string, issue: LinearIssue | CoreIssue, repositoryId: string) => void
