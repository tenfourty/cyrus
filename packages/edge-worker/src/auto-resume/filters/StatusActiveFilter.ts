 ...
 * what `AgentSessionManager.getActiveSessions()` did historically — but that
 * makes the skip silent. Routing the check through the filter pipeline
 * surfaces every non-active session in the run summary's `skipped` array
 * with reason `status-not-active`, so an operator scanning the journal can
 * tell the difference between "no in-flight work to resume" and "we have
 * sessions whose status looks like it was never updated when it should
 * have been."
 *
 * Pair with `EdgeWorker.resumeAgentSession`'s `markSessionResuming()` call
 * (which prevents stale `Complete` from carrying through a new turn).
 */
