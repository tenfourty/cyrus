	...
	private async handleGitHubWebhook(
		...
		try {
			...
			if (!repository) {
				...
				// an explicit @-mention (using the bot identity resolved at startup —
				// not the env-var-based username the upstream snippet originally used),
				// or a pull_request_review requesting changes.
				const wasMentioned =
					!!botIdentity?.username &&
					commentBody.includes(`@${botIdentity.username}`);
...
${taskSection}`;
	...
	private async handleIssueStateChangeMessage(
		...
		// `repositories` is also threaded through so the per-repo
		// `cyrus-teardown.sh` auto-detection (upstream feature) runs against
		// each repo before its worktree is removed.
	...
	private async handleAgentSessionCreatedWebhook(
		...
		// Guard against two runners working the same issue concurrently. Linear
		// can emit two `created` webhooks for one issue (delegation + @mention),
		// each its own agentSession; both resolve the same issue worktree, so
		// without this they would mutate one tree/branch at once. The per-issue
		// lock makes the check-and-reserve atomic against truly-simultaneous
		// webhooks; `issuesInitializing` bridges the window until the first
		// session is registered Active (after its worktree is created).
	...
	private async foldDuplicateSessionIntoActive(
		...
		// Route the duplicate's comment into the live session's running runner.
		let delivered = false;
		const trimmedComment = commentBody?.trim();
		if (targetSessionId && trimmedComment) {
			const target = this.agentSessionManager.getSession(targetSessionId);
			const runner = target?.agentRunner;
			if (
				runner?.isRunning() &&
				...
				try {
					runner.addStreamMessage(trimmedComment);
					delivered = true;
				} catch (error) {
					this.logger.debug(
		...
		try {
			...
			this.logger.debug(
				`Could not post decline response to duplicate session ${duplicateSession.id}: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
	...
	private async initializeAgentRunner(
		...
		try {
			...
			const abortReason = shouldAbortSpawn({
				...
				agentSessionManager,
				logger: this.logger,
				isDraining: () => this.drainController.isDraining(),
			});
			if (abortReason) {
				log.info(
...
${input.userComment}
	...
	private buildAgentContextBlock(): string {
		...
		const githubBot =
			this.gitHubBotIdentity?.username || process.env.GITHUB_BOT_USERNAME || "";
		const gitlabBot =
			this.gitLabBotIdentity?.username || process.env.GITLAB_BOT_USERNAME || "";
...
${input.userComment}
	...
	private async buildAgentRunnerConfig(
		...
		// Resolve plugins once — combining the standard plugin set with the
		// per-session sibling-skill plugins for multi-repo sessions (tenfourty
		// feature) — and derive the per-session scoped skill allow-list from
		// that same snapshot (upstream skill-scope feature).
		const plugins = [
