...
${taskSection}`;
	...
	private async ensureFreshLinearToken(
		...
		try {
			...
			// so the next call in *this process* observes a known-future expiry.
			// NOTE: this fires once per process, not once per token overall — only
			// this refresh path's callback writes linearTokenExpiresAt. The other
			// token-minting sites (RefreshTokenCommand, SelfAuthCommand, and
			// SharedApplicationServer.startOAuthFlow) don't set it, so a token
			// minted by any of those still reads as unknown-expiry and re-triggers
			// this branch on the next call in a new process (tracked separately).
