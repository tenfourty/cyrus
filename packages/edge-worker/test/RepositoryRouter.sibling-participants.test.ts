...
describe("RepositoryRouter — siblingParticipants augmentation", () => {
	describe("project-based routing", () => {
		...
		it("returns only the primary when siblingParticipants is unset", async () => {
			const repoA = buildRepo("repoA", { projectKeys: ["Cove"] });
			const coveOvh = buildRepo("repoB");
		...
		it("silently skips sibling IDs that do not resolve to a registered repo", async () => {
			const repoA = buildRepo("repoA", {
				projectKeys: ["Cove"],
				siblingParticipants: ["repoB", "ghost-repo"],
			});
			const coveOvh = buildRepo("repoB");
	...
	describe("label-based routing", () => {
		it("appends siblings to the label-matched primary", async () => {
			const repoA = buildRepo("repoA", {
				routingLabels: ["repo:repoA"],
				siblingParticipants: ["repoB"],
			});
			const coveOvh = buildRepo("repoB");

			const { router } = buildRouter({ issueLabels: ["repo:repoA"] });
			const result = await router.determineRepositoryForWebhook(
				buildWebhook(),
				[repoA, coveOvh],
			);
	...
	describe("description-tag routing (explicit override)", () => {
		it("does NOT augment with siblingParticipants when user supplied [repo=...] tag", async () => {
			const repoA = buildRepo("repoA", {
				projectKeys: ["Cove"],
				siblingParticipants: ["repoB", "repoDeploy"],
			});
			const coveOvh = buildRepo("repoB");
			...
			const { router } = buildRouter({
				issueProject: "Cove",
				issueDescription: "[repo=repoA] only this one please",
			});
			const result = await router.determineRepositoryForWebhook(
				buildWebhook(),
				[repoA, coveOvh, coveDeploy],
			);

			expect(result.type).toBe("selected");
			if (result.type !== "selected") return;
			// Tag explicitly named only `repoA` — siblings must NOT be appended
			expect(result.repositories.map((r) => r.id)).toEqual(["repoA"]);
		});
	});
	...
	describe("deduplication", () => {
		it("does not duplicate a sibling that the routing path already returned", async () => {
			// Both repoA and repoB have label `shared` — both match label routing.
			// repoA also lists repoB as a sibling. Result must contain each once.
			const repoA = buildRepo("repoA", {
				routingLabels: ["shared"],
				siblingParticipants: ["repoB"],
			});
			const coveOvh = buildRepo("repoB", { routingLabels: ["shared"] });
