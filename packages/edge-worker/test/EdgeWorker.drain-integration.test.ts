/**
 * EdgeWorker — drain integration tests (Task 5)
 *
 * Covers:
...
describe("EdgeWorker — drain integration", () => {
	...
	it("shouldAbortSpawn is called with isDraining accessor at both spawn sites", async () => {
		const { shouldAbortSpawn } = await import("../src/shouldAbortSpawn.js");
		const spawnSpy = vi.mocked(shouldAbortSpawn);

		edgeWorker = new EdgeWorker(baseConfig);

		// The drain controller starts in 'running' state — isDraining() = false
	...
	it("auto-resume posts warning for sessions with lastInFlightToolUses and clears marker", async () => {
		...
		const session = {
			...
			createdAt: Date.now(),
			updatedAt: Date.now(),
			lastInFlightToolUses: [
				{ id: "t1", name: "Bash", killedAt: new Date().toISOString() },
				{ id: "t2", name: "Edit", killedAt: new Date().toISOString() },
			],
		};
		...
		const warningActivity = thoughtActivities.find((a) =>
			a.body.includes("may have partially completed"),
		);
		expect(warningActivity).toBeDefined();
		expect(warningActivity!.body).toContain("`Bash`");
		expect(warningActivity!.body).toContain("`Edit`");
