	...
	it("forwards extra additionalAllowedDirectories alongside secondary source paths", () => {
		...
		const session = {
			workspace: {
				...
				isGitWorktree: true,
				repoPaths: {
					"cove-id": "/worktrees/CEE-300/cove",
					"cove-ovh-id": "/worktrees/CEE-300/cove-ovh",
				},
			},
		} as CyrusAgentSession;
		...
		expect(result).toContain("/src/cove-ovh");
		expect(result).toContain("/extra/parent-feedback-dir");
		expect(result).toContain("/src/cove");
		expect(result).toContain("/worktrees/CEE-300/cove");
		expect(result).toContain("/worktrees/CEE-300/cove-ovh");
	});
});
