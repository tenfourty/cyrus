...
describe("RepositoryRouter — multi-repo description tag", () => {
	...
	it("returns all repos listed in a single comma-separated [repo=a,b] tag", async () => {
		const router = buildRouter("Fix this: [repo=repoA,repoB]\n\nDetails.");
	...
	it("returns all repos when multiple separate [repo=...] tags are present", async () => {
		const router = buildRouter("[repo=repoA] and also [repo=repoDeploy]");
		const result = await router.determineRepositoryForWebhook(buildWebhook(), [
			repoA,
			coveOvh,
			coveDeploy,
	...
	it("applies a single trailing #branch to all comma-separated repos in a bracketed tag", async () => {
		const router = buildRouter("[repo=repoA,repoB#feature-x]");
		const result = await router.determineRepositoryForWebhook(buildWebhook(), [
			repoA,
			coveOvh,
		]);
		...
		expect(result.baseBranchOverrides?.get("repoA")).toBe("feature-x");
		expect(result.baseBranchOverrides?.get("repoB")).toBe("feature-x");
	});

	it("supports per-repo branch overrides via multiple separate bracketed tags", async () => {
		const router = buildRouter(
			"[repo=repoA#feature-x] needs to be paired with [repo=repoB#main]",
		);
		const result = await router.determineRepositoryForWebhook(buildWebhook(), [
			repoA,
			coveOvh,
		...
		expect(result.repositories.map((r) => r.id)).toEqual(["repoA", "repoB"]);
		expect(result.baseBranchOverrides?.get("repoA")).toBe("feature-x");
		expect(result.baseBranchOverrides?.get("repoB")).toBe("main");
	});

	it("returns single repo when only one tag is supplied (backward compat)", async () => {
		const router = buildRouter("[repo=repoA]");
		const result = await router.determineRepositoryForWebhook(buildWebhook(), [
			repoA,
			coveOvh,
		]);
		...
		expect(result.type).toBe("selected");
		if (result.type !== "selected") return;
		expect(result.repositories.map((r) => r.id)).toEqual(["repoA"]);
	});

	it("ignores unknown repo names in the tag and returns only matched repos", async () => {
		const router = buildRouter("[repo=repoA,nonexistent,repoB]");
		const result = await router.determineRepositoryForWebhook(buildWebhook(), [
			repoA,
			coveOvh,
		]);
