...
describe("RunnerConfigBuilder additionalDirectories (multi-repo skill discovery)", () => {
	it("registers each multi-repo sub-worktree as an additional directory", async () => {
		...
		// `resolveSessionWorkingDirectory` resolves cwd to the PRIMARY repo's
		// worktree for multi-repo sessions (so the agent's `git`/shell commands
		// operate on a real repo rather than the parent dir of N worktrees).
		// `additionalDirectories` then enumerates the OTHER worktrees — repo-a
		// is filtered out because it is now the cwd.
		expect(config.workingDirectory).toBe("/ws/root/repo-a");
