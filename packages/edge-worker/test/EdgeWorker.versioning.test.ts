...
{{issue_description}}`
    ...
    const extractVersionTag = (edgeWorker as any).extractVersionTag.bind(edgeWorker)
...
Repository: {{repository_name}}`
    ...
    expect(logSpy).toHaveBeenCalledWith('[EdgeWorker] Prompt template version: debugger-v2.1.0')
  })
...
Repository: {{repository_name}}`
    ...
    const versionLogs = logSpy.mock.calls.filter(call => 
      call[0] && call[0].includes('Prompt template version:')
    )
