...
describe("EdgeWorker - session pre-warm cwd", () => {
	...
	it("warms a multi-repo session with the primary repo subdir as cwd (not the workspace root)", async () => {
		...
		injectSession({
			...
			workspace: {
				path: "/test/workspaces-a/ENG-432",
				repoPaths: { "repo-a": "/test/workspaces-a/ENG-432/repoA" },
			},
			issueContext: { issueIdentifier: "ENG-432" },
		});
		...
		// The regression: this used to be "/test/workspaces-a/ENG-432" (root).
		expect(passed.options.cwd).toBe("/test/workspaces-a/ENG-432/repoA");
	...
	it("skips a candidate whose transcript is gone without warming it or throwing", async () => {
		...
		injectSession({
			...
			status: AgentSessionStatus.Active,
			workspace: {
				path: "/test/workspaces-a/ENG-9",
				repoPaths: { "repo-a": "/test/workspaces-a/ENG-9/repoA" },
			},
			issueContext: { issueIdentifier: "ENG-9" },
		});
