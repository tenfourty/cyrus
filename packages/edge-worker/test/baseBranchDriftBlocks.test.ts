...
const repoA: any = { id: "repo-a", name: "cove", baseBranch: "main" };
const repoB: any = { id: "repo-b", name: "cove-ovh", baseBranch: "develop" };
...
describe("computeResumeBaseBranchDriftBlocks", () => {
	...
	it("returns one block when the single repo's base drifted", async () => {
		...
		expect(blocks[0]).toContain("<repository>cove</repository>");
	...
	it("emits one block per drifted repo for multi-repo sessions", async () => {
		const gitService = {
			checkBaseBranchDrift: vi.fn(async (path: string) => {
				if (path === "/worktrees/repo-a/AIN-1") {
					return { commitCount: 4, branchName: "main" };
				}
				if (path === "/worktrees/repo-b/AIN-1") {
		...
		const blocks = await computeResumeBaseBranchDriftBlocks({
			session: makeSession({
				workspace: {
					...
					isGitWorktree: true,
					repoPaths: {
						"repo-a": "/worktrees/repo-a/AIN-1",
						"repo-b": "/worktrees/repo-b/AIN-1",
					},
				},
			}),
			primaryRepo: repoA,
		...
		expect(blocks[1]).toContain("<repository>cove-ovh</repository>");
	...
	it("emits no block for the clean repo and one block for the drifted sibling in multi-repo", async () => {
		const gitService = {
			checkBaseBranchDrift: vi.fn(async (path: string) => {
				if (path === "/worktrees/repo-a/AIN-1") return null;
				if (path === "/worktrees/repo-b/AIN-1") {
		...
		expect(blocks).toHaveLength(1);
		expect(blocks[0]).toContain("<repository>cove-ovh</repository>");
	});
