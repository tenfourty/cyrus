 ...
 *
 * Users can prefix a Slack mention with `[repos=cove,cove-ovh]` (or
 * unbracketed `repos=cove,cove-ovh`) to narrow the repository set the
...
describe("SlackChatAdapter — first-message repo tag", () => {
	const allPaths = ["/repos/cove", "/repos/cove-ovh", "/repos/cove-deploy"];

	it("extractTaskInstructions strips a leading [repos=...] tag", () => {
		const adapter = new SlackChatAdapter(buildProvider(allPaths));
		const event = buildEvent("[repos=cove,cove-ovh] update the runbook please");
	...
	it("buildSystemPrompt filters Repository Access to tagged repos only", () => {
		const adapter = new SlackChatAdapter(buildProvider(allPaths));
		const event = buildEvent("[repos=cove,cove-ovh] do the thing");
		const prompt = adapter.buildSystemPrompt(event);
		// Tagged repos must appear
		expect(prompt).toContain("/repos/cove");
		expect(prompt).toContain("/repos/cove-ovh");
		// Untagged repo must NOT appear (basename match — full path filtered out)
		expect(prompt).not.toContain("/repos/cove-deploy");
	});
	...
	it("buildSystemPrompt shows all configured paths when no tag is present", () => {
		...
		expect(prompt).toContain("/repos/cove");
		expect(prompt).toContain("/repos/cove-ovh");
		expect(prompt).toContain("/repos/cove-deploy");
	});
	...
	it("buildSystemPrompt matches repo by basename of the path", () => {
		const adapter = new SlackChatAdapter(buildProvider(allPaths));
		const event = buildEvent("[repo=cove-ovh] just this one");
		const prompt = adapter.buildSystemPrompt(event);
		expect(prompt).toContain("/repos/cove-ovh");
		expect(prompt).not.toContain("/repos/cove-deploy");
		// Ensure we're not matching by substring — `/repos/cove` should NOT appear
		// because the user explicitly tagged only `cove-ovh`. The basename of
		// `/repos/cove` is `cove`, not `cove-ovh`.
		expect(prompt.includes("/repos/cove\n")).toBe(false);
		expect(prompt.includes("/repos/cove ")).toBe(false);
	});
});
