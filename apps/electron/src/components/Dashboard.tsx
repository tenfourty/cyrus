...
interface DashboardProps {
  ...
  onIssueAction: (issueId: string, action: 'pause' | 'restart' | 'copy' | 'open') => void
}
...
export function Dashboard({ issues, selectedIssueId, onSelectIssue, onIssueAction }: DashboardProps) {
  const selectedIssue = selectedIssueId ? issues.get(selectedIssueId) : null
  
  // Group issues by status
  const activeIssues = Array.from(issues.values()).filter(i => 
    i.status === 'active' || i.status === 'waiting' || i.status === 'paused'
  )
  const completedIssues = Array.from(issues.values()).filter(i => 
        ...
        <ScrollArea className="flex-1">
            ...
            {activeIssues.map(issue => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-lg">{getStatusLabel(issue.status)}</span>
          ...
          {completedIssues.length > 0 && (
            <>
              ...
              <div className="p-2">
                <p className="text-sm font-semibold text-muted-foreground px-3 mb-2">COMPLETED</p>
        ...
        {selectedIssue ? (
            ...
            <div className="p-4 border-b">
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                <span className="flex items-center gap-1">
                  ...
                  {selectedIssue.status === 'active' ? 'Active' : 
                   selectedIssue.status === 'waiting' ? 'Waiting' :
                   selectedIssue.status === 'paused' ? 'Paused' :
                   selectedIssue.status === 'completed' ? 'Completed' : 'Failed'}
                </span>
              ...
              <div className="space-y-4">
                {selectedIssue.logs?.map((log, index) => (
                  ...
                  <div className="text-center text-muted-foreground py-8">
                    <p>No activity yet</p>
                    <p className="text-sm mt-2">Waiting for Claude to start processing...</p>
                  </div>
                )}
              </div>
            </ScrollArea>
          ...
          <div className="flex-1 flex items-center justify-center text-muted-foreground">
            <div className="text-center">
              ...
              <p className="text-sm">Active issues will appear here when assigned</p>
            </div>
          </div>
        )}
      </div>
