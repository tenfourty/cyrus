import { app, BrowserWindow, ipcMain, shell, protocol, Tray, Menu, dialog } from 'electron'
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('cyrus', process.execPath, [process.argv[1]])
  }
} else {
...
ipcMain.handle('validate-paths', async (_, paths: { claudePath?: string, repository?: string, workspaceBaseDir?: string }) => {
  ...
  if (paths.workspaceBaseDir) {
    const expandedPath = paths.workspaceBaseDir.replace(/^~/, app.getPath('home'))
