...
describe("buildEdgeWorkerConfig", () => {
	...
	it("splits LINEAR_ALLOWED_TOOLS env into linearAllowedTools array", () => {
		const result = buildEdgeWorkerConfig({
			edgeConfig: baseEdgeConfig,
			env: { LINEAR_ALLOWED_TOOLS: "Read, Write , Bash" },
			repositories,
			cyrusHome: "/tmp/cyrus-home",
		});

		expect(result.linearAllowedTools).toEqual(["Read", "Write", "Bash"]);
	});
