/**
 * Application — graceful-drain signal-handler tests.
 *
 * Covers the SIGTERM/SIGINT contract introduced with graceful drain:
 *   - first SIGTERM enters drain mode and awaits the DrainOutcome before
 *     stopping the worker (so the outcome reaches EdgeWorker.stop and
 *     force-kill markers get persisted),
 *   - a second SIGTERM while a drain is in flight calls abortDrain() and
 *     falls through to immediate shutdown,
...
function makeAppWithSignalHandlers(): {
	...
	(app as any).drainShutdownPromise = undefined;
...
describe("Application signal-handler drain behaviour", () => {
	...
	it("SIGINT bypasses drain entirely and shuts down immediately", async () => {
		...
		expect(stopFn).toHaveBeenCalledOnce();
		// SIGINT calls shutdown() with no outcome → worker.stop(undefined)
