...
import type { Env, EdgeEvent, LinearWebhook } from '../types'
...
export class WebhookSender {
  ...
  async sendWebhookToWorkspace(event: EdgeEvent, workspaceId: string): Promise<number> {
    ...
    console.log(`Delivered webhook to ${successCount}/${edgeWorkers.length} edge workers for workspace ${workspaceId}`)
    return successCount
  }
   ...
   */
  private async deliverWebhookToEdgeWorker(worker: StoredEdgeWorker, event: EdgeEvent): Promise<void> {
    const maxRetries = 3
    let attempt = 0
  ...
  private async makeWebhookRequest(worker: StoredEdgeWorker, event: EdgeEvent): Promise<void> {
    ...
    const signature = this.generateWebhookSignature(body, timestamp, worker.webhookSecret)
   ...
   */
  private generateWebhookSignature(body: string, timestamp: string, secret: string): string {
