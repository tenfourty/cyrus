...
### Fixed
- Cyrus no longer hangs indefinitely on a clarifying question nobody answers. When the agent asks a question (via Linear's select prompt) and no one responds within a timeout (`CYRUS_ELICITATION_TIMEOUT_MS`, default 15 minutes), the agent is now told no response arrived and proceeds with a sensible default instead of leaving the turn stuck open — so an unattended session can't wedge on an unanswered prompt. Answering normally is unchanged; a late answer that arrives after the timeout is ignored (the turn already moved on).
...
- **GitHub bot identity is resolved automatically.** Self-hosted users running a GitHub App no longer need to set `GITHUB_BOT_USERNAME` for the self-comment filter to work — the App slug (e.g. `agentHost-app[bot]`) is fetched once at startup via `GET /app`. PAT deployments resolve via `GET /user`. The agent context block shipped to skills now uses the resolved bot username instead of the raw env value.
