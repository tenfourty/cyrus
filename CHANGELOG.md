...
### Changed
- **Long-running sessions auto-compact earlier so they stay survivable.** Claude Code's default auto-compact trigger reserves only ~13k tokens of headroom before compacting — roughly 93.5% of a 200k window. That margin is too thin for tool-heavy turns: AIN-555 wedged at 96% of a 1M-token window after two SDK-triggered auto-compactions still couldn't keep pace with growth, and every subsequent resume rehydrated the over-budget transcript and failed with "Prompt is too long". Cyrus now sets `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=50` by default in every Claude session subprocess, so auto-compaction fires at 50% of the model's context window and leaves comfortable margin for the next turn to complete. Operators can override per-host (`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` in `~/.cyrus/.env`) or per-repo (same var in the repo's `.env` file). Trade-off: compactions are more frequent (each costs roughly the time + tokens of one summarization model call), but sessions no longer wedge themselves.
