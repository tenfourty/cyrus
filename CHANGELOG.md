...
### Changed
- **Updated Claude SDK dependencies** - Updated `@anthropic-ai/claude-agent-sdk` to v0.2.63 and `@anthropic-ai/sdk` to v0.78.0. Notably, MEMORY now works consistently when using Claude — an unbounded memory growth bug in long-running sessions (caused by message UUID tracking never evicting old entries) was fixed in v0.2.51. Also adds `getSessionMessages()` for reading session history, `listSessions()` for discovering past sessions, and improved `task_progress` events for real-time background agent reporting. See [claude-agent-sdk changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for details. ([CYPACK-859](https://linear.app/ceedar/issue/CYPACK-859), [#917](https://github.com/ceedaragents/cyrus/pull/917))
