...
### Fixed
- A stuck, crashed, or errored agent session can now be recovered by re-prompting it in Linear — no full restart required. Sessions whose runner died out of band (crash/OOM) or ended on an error (e.g. a gateway 429) are reconciled so a new prompt cleanly starts a fresh runner instead of silently folding into a dead session. Orphaned runner processes from errored sessions are now cleaned up. (To recover one stuck session: **stop it in Linear, then re-prompt** — with warm sessions enabled, send stop twice within 10s to force a full stop rather than an interrupt.) ([#1](https://github.com/tenfourty/cyrus/pull/1))
...
- Session pre-warm (`CYRUS_ENABLE_WARM_SESSIONS=1`) now actually warms sessions on multi-repo deployments. It was resuming each candidate with the workspace root as the working directory instead of the primary repo's worktree (the directory the live runner uses), so Claude Code looked for transcripts under the wrong location and every candidate failed with "No conversation found" — warm mode silently readied 0 sessions. Pre-warm now uses the same working directory as the live runner, candidates with on-disk transcripts warm successfully, a candidate whose transcript is genuinely gone is skipped (the live fresh-start fallback covers it), and the completion summary reports ready/failed/skipped counts at info level so a 0-warmed outcome is visible without debug logging.
...
### Added (auto-compact configurability)
...
- **Pre-turn `/compact` guard at Claude session resume.** Cyrus now checks the session's last-recorded context utilization at every Claude resume entry. When the value exceeds the configured `autoCompactThresholdPercent` (the same knob that drives the SDK's mid-turn auto-compaction), Cyrus runs a one-shot `/compact` against the existing `claudeSessionId` *before* forwarding the user's actual prompt to a fresh runner. Closes the residual gap where the SDK's auto-compaction (which only fires mid-stream during a turn) can't help a session that ended above threshold and is about to rehydrate an over-budget transcript. Disabled when no threshold is configured (operator opt-in); silently best-effort — if the compact itself fails (e.g. the session is already past 100% of the window), the resume proceeds anyway and the user's prompt is what surfaces the wall. Model context window detected from `session.metadata.model` with a 1M-token window recognized for Opus `[1m]` variants and a 200k fallback for everything else.
...
- **Auto-resume skip-breakdown log now splits dominant reasons from actionable ones.** Skip reasons with a count of 100 or more (typically `status-not-active` on long-lived installs that accumulate completed sessions) print on a separate line tagged `(historical, expected)` so the `actionable` line stays readable when an actual ops issue shows up (e.g. `worktree-missing=2, hold-label=1`). No retention policy yet — the file size still grows linearly with session count — but the journal stays useful in the meantime.
...
- **GitHub bot identity is resolved automatically.** Self-hosted users running a GitHub App no longer need to set `GITHUB_BOT_USERNAME` for the self-comment filter to work — the App slug (e.g. `cove-agent-app[bot]`) is fetched once at startup via `GET /app`. PAT deployments resolve via `GET /user`. The agent context block shipped to skills now uses the resolved bot username instead of the raw env value.

### Added
- **Graceful drain on shutdown.** Cyrus now defers SIGTERM until each active Claude session reaches a tool-call boundary (no `tool_use` waiting on a `tool_result`). Restart no longer interrupts in-flight tool execution like `git push`, `cargo build`, `cove deploy`, or MCP HTTP requests; LLM completions may still get interrupted but resume cleanly via session-id resume. Drain has a configurable hard cap (default 30 minutes; set `CYRUS_DRAIN_HARD_CAP_MS=<ms>` to change, `0` to disable and restore prior immediate-shutdown behavior) and a per-session cap (default 20 minutes; `CYRUS_DRAIN_PER_SESSION_CAP_MS`). When the hard cap fires, force-killed sessions persist a marker that triggers a Linear warning activity on the next auto-resume so operators know which tool calls may have partially completed. New endpoints `POST /admin/drain` (initiate without killing the process — fires SIGTERM after drain completes) and `GET /admin/drain/status` (live per-session pending tool list, age, hard-cap remaining). A second SIGTERM during drain aborts and proceeds to immediate shutdown; SIGINT continues to bypass drain entirely. Webhooks for new sessions and prompts (other than `signal: stop`, which is always accepted so operators can cancel stuck sessions) return `503 Retry-After: 30` during drain so Linear's native retry picks them up after restart. Drain only waits on Claude sessions; Codex/Cursor/Gemini sessions fall through to per-session cap until their runners learn to emit synthetic `tool_result` on subprocess abort. **Operator note:** ensure systemd `TimeoutStopSec` is at least `CYRUS_DRAIN_HARD_CAP_MS + 30s` (default unit ships with 30s, raise it before relying on drain in production).
...
- **Per-repo `siblingParticipants` config field** — Each repository entry can now declare an optional `siblingParticipants: string[]` (repo IDs) so that when this repo is matched as primary by routing (label, project, team, or catch-all), the listed siblings are auto-appended to the session as participants. Useful for setups where a primary code repository always needs a sibling operations or shared-library repository (for runbooks, deploy wrappers, shared types, etc.) without requiring users to add `[repo=a,b]` to every issue body. One-way directional — set on repo A only and sessions routed to repo B stay single-repo unless mirrored. Description-tag routing (`[repo=...]`) is treated as an explicit override and skips sibling augmentation. Unknown sibling IDs are logged and skipped.
- **Multi-select repository picker** — When Cyrus posts a repository-selection elicitation in Linear (because no routing rule matched), the user can now reply with a comma-separated list of repo names/URLs (e.g. `repo-a, repo-b`) to select MULTIPLE repositories for the session. Previously the response was treated as a single repo only and additional values were dropped. Single-value responses still work the same way (backward compat). Falls back to the first pending repo when the response matches nothing — same fallback behavior as before.
- **Slack first-message repo tag** — Slack mentions can now be prefixed with `[repos=a,b]` (or unbracketed `repos=a,b`, plus singular `[repo=name]` / `repo=name`) to narrow the repository set the agent operates on for the thread. The tag is parsed out before the message is shown to the agent (so it doesn't pollute task instructions) and is used to filter the Repository Access section of the Slack chat system prompt to only the named repos. Matching is by basename of the configured repo path. When the tag references unknown repo names, the chat falls back to showing all configured paths so the conversation stays usable. Slash-command (`/cyrus repos a,b`) form is not yet implemented and would require Slack app config — first-message tag is the supported entry point for now.
...
- **Resumed sessions are notified when their base branch moved during dormancy.** When Cyrus resumes a session (user comment, auto-resume on startup, or manual re-ping), the worktree is now fetched and compared against the remote base branch before the agent is handed the prompt. If `origin/<base>` advanced since the last fetch, a `<base_branch_update>` block is prepended to the prompt with the new commit count and the same rebase guidance the live GitHub-push path streams to active sessions. Fixes the long-pause scenario where an agent picks up an issue Monday after `main` advanced over the weekend and only discovers conflicts at PR time. Multi-repo sessions check each participating worktree independently and emit one block per drifted repo. Detection uses a before/after SHA delta of the local tracking ref so the agent is not pinged for divergence it already saw at session creation. The check is best-effort — any git failure logs a warning and falls through without blocking resume.

### Fixed
- **Stopped agent sessions no longer silently drop subsequent prompts.** When a user stopped an in-flight agent session via the Linear UI and later sent a new prompt to continue the conversation, Cyrus aborted the new spawn with "stop was requested for this session before the runner could start" — and kept doing it for every subsequent prompt until the process restarted. The in-memory stop flag had no cleaner outside the runner-result and issue-terminal paths, neither of which fires in the stop-then-prompt-again flow. The flag is now cleared at the top of the user-prompt entry path so the user's next prompt actually runs.
...
- **Resume-time base-branch drift check now reports correctly on busy Cyrus instances.** The drift check shipped in the prior entry compared `origin/<base>`'s SHA before and after this fetch, which silently produced no notification whenever parallel cyrus traffic (other webhooks, sibling sessions, push handlers running against the same bare repo) had already advanced `origin/<base>` before the resume's fetch ran. Resumed sessions on busy instances inherited stale bases with no signal — observed live as 9-commit-behind worktrees that resumed without a `<base_branch_update>` block. The check now asks git directly: `merge-base HEAD origin/<base>` plus `rev-list --count <fork>..origin/<base>`, which answers "is this branch behind upstream?" regardless of what other processes did to the bare repo. Live GitHub-push notifications are unchanged — they build the block from the webhook payload and never call this function.
- **Cyrus sessions can now maintain the auto-memory `MEMORY.md` index.** The broad home-directory Read deny (which protects `~/.claude.json` API tokens, session transcript JSONLs, and other home-dir secrets) silently caught the per-repo auto-memory directory under `~/.claude/projects/<encoded-repo-path>/memory/`, so agents successfully created new memory entry files (Write to a new path is fine) but could not Read+Edit the `MEMORY.md` index to add their pointer line — entries were left orphaned and invisible to future sessions. Sessions now carve out the per-repo auto-memory directory in the runner's `allowedDirectories`, computed by encoding the bare repository path the same way Claude Code does (each `/` and `.` becomes `-`, e.g. `/root/.cyrus/repos/cove` → `~/.claude/projects/-root--cyrus-repos-cove/memory/`). The deny still covers `~/.claude.json`, `~/.claude/agents`, every other project's directory, and the session transcript JSONLs that sit alongside the memory subdirectory.
- **Persistence file is now written atomically and shutdown is idempotent.** Cyrus's `~/.cyrus/state/edge-worker-state.json` was previously written via plain `writeFile`, which is truncate-then-write — a process killed mid-write (or two writers racing on the same path) left the file empty or partial. Combined with an unguarded `shutdown()` that ran twice when `systemctl restart` delivered SIGTERM more than once, restarts during active sessions could leave the state file empty and silently drop every in-flight session, defeating auto-resume. Saves now write to a per-process tmp file and atomically `rename()` into place; shutdown is guarded so concurrent signal handlers (or the uncaughtException path) all observe the same single drain. `loadEdgeWorkerState` also tolerates a zero-byte file from old crashes (returns `null` silently instead of logging a JSON parse error) and cleans up any stale `.tmp.*` siblings at load time.
...
- **GitHub App self-comment filter now activates correctly.** The previous identity-resolution code at `GET /app` authenticated with an installation access token, which GitHub rejected with 401 "A JSON web token could not be decoded" — the resolver fell back silently to the `GITHUB_BOT_USERNAME` env override on every restart, and self-hosted GitHub App deployments that hadn't set that env never had the in-process self-comment safety net active. GitHub App identity is now resolved using a fresh App JWT (signed with the App's private key), which is the credential `GET /app` actually requires. App installations are already loop-safe by structure (the `<slug>[bot]` author can't collide with a human commenter), but having the resolved identity active means the self-skip filter and the agent-context block ship the real bot account name instead of empty defaults.
- **GitLab bot-identity resolver now respects the self-hosted GitLab host.** The resolver call shipped in the previous Unreleased entry defaulted to `https://gitlab.com` and was never passed the API base URL derived from the configured `repos[].gitlabUrl`. On any self-hosted GitLab deployment the startup call hit `gitlab.com/api/v4/user` with a token issued by the self-hosted host, got a (correct) 401, and the fail-closed branch deregistered the GitLab webhook handler — silently disabling all MR interactivity for self-hosted Cyrus users. The base URL is now derived once and shared between `GitLabCommentService` and the resolver, so both call the same host.
