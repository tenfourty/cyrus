...
### Added
- Cyrus can now pick up in-flight work on its own after a restart, instead of waiting for you to nudge every issue by hand. Turn it on per repository with `autoResumeOnStartup: true` (off by default). Sessions you stopped, sessions on issues that were closed, cancelled, or moved back to the backlog while Cyrus was down, sessions whose worktree is gone, and sessions on issues labelled `cyrus:hold` are all left alone. Resumed sessions post a short note so you can see what happened. Tuning lives under `autoResume` in your config: how many resume at once, how much they stagger, how old is too old, how many times a failing session is retried before Cyrus gives up on it, the hold label, and whether to post the note.

### Fixed
- Cyrus no longer loses its record of in-flight sessions when it is killed part-way through saving, or when a restart delivers two shutdown signals in a row. Pressing Ctrl-C a second time (or sending a second shutdown signal) now forces an immediate exit rather than waiting on the first one, and a shutdown that stalls no longer hangs the process indefinitely.
