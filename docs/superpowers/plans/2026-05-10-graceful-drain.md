...
**Goal:** Make `systemctl restart cyrus.service` (SIGTERM) wait for in-flight tool calls to finish before exiting, so deploys never kill `git push`/`cove deploy`/`cargo build`/MCP HTTP calls mid-execution.
...
On staging cove-agent or local:
